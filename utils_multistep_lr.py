...
def train(model_class, train_gen, valid_gen, train_batch_size, \
    ...
    print("begin building saver")
    saver = tf.train.Saver(max_to_keep=10)
    variables = tf.contrib.framework.get_variables_to_restore()
    variables_to_restore =  [v for v in variables if ( len(v.name.split('/'))==1 ) or ( len(v.name.split('/'))>1  and (v.name.split('/')[0]!='Layer8' ) and (v.name.split('/')[0]!='Layer9' ) and (v.name.split('/')[0]!='Layer10' ) and (v.name.split('/')[0]!='Layer11' ) and (v.name.split('/')[0]!='ip' ))]
    saver2 = tf.train.Saver(variables_to_restore)
    print("begin running to session")
    with tf.Session() as sess:
        sess.run(init_op)
        print("after initialization")
        if load_path is not None:
            saver.restore(sess, load_path)
            print("load the old path")
